...
const ManifestSchema = z.object({
  namingExpressionProbe: z.object({
    aestheticEvidence: z.array(z.string().min(1)).min(1),
  }),
});
...
const ExpressionEnvelopeSchema = z.object({
  version: z.literal("work-cell.naming-expression-probe.v1"),
  runId: z.string().min(1),
  treatment: z.object({
    direct: z.object({ candidates: z.array(CandidateSchema).min(1) }),
    projected: z.object({ candidates: z.array(CandidateSchema).min(1) }),
  }),
});
...
async function main(args: string[]): Promise<void> {
  ...
  if (!expressionArg || !manifestArg) {
    throw new Error("usage: bun src/naming-surface-gate-probe.ts <naming-expression.json> <manifest.json>");
  }
  ...
  const candidates = [...expression.treatment.direct.candidates, ...expression.treatment.projected.candidates]
    .filter((candidate, index, all) => all.findIndex((entry) => entry.name.toLocaleLowerCase() === candidate.name.toLocaleLowerCase()) === index)
    .map((candidate) => ({ name: candidate.name, spokenForm: candidate.spokenForm }));
  ...
  const results = await Promise.all(seats.map(async (seat) => {
    const result = await generateText({
      ...
      system: [
        ...
        "The Principal's examples are contrastive observations, never rules about language, length, origin, character count, or naming mechanism.",
        "## Seat",
        seat.instructions,
      ].join("\n"),
      prompt: [
        "## Principal aesthetic evidence",
        JSON.stringify(manifest.namingExpressionProbe.aestheticEvidence),
  ...
  await writeFile(output, `${JSON.stringify({
    version: "work-cell.naming-surface-gate-probe.v1",
    runId,
