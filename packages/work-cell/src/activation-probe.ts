...
async function main(args: string[]): Promise<void> {
  ...
  if (!manifestArg) throw new Error("usage: bun src/activation-probe.ts <manifest.json> (--scale <n> | --baseline) [--propagate-shapes] [--working-set <n>]");
  ...
  if (!process.env.DEEPSEEK_API_KEY) throw new Error("DEEPSEEK_API_KEY is required for a live activation probe");
