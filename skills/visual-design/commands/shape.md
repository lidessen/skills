...
1. Inspect the actual object, neighboring surfaces, product/content sources,
   ...
   when possible. State which observations are direct and which are inferred.
2. Read `references/presentation-model.md`. Inventory the content and actions,
   including their relationships, authority, variability, and states. Define
   the intended attention path: orientation, primary action, supporting
   judgment, context, and recovery signals. Do not ask layout or color to repair
   unresolved content priority.
3. Name the audience and the concrete perception, decision, or action the
   artifact must enable. Identify the present contradiction at the level of the
   whole experience, not only the most visible component.
4. Recover inherited identity and hard constraints. Existing brand, platform,
   accessibility, content authority, and production conditions are inputs; do
   not silently replace them with a preferred aesthetic. A brief description
...
Content structure and variability:
Intended attention path:
Observed state and principal contradiction:
Perceptual relation to make present:
...
Observation that would reject this direction:
```
