...
1. Read the owning direction or design system, the real implementation, rendered
   ...
   broader design change instead of polishing around it. If rendered evidence
   or inspection capability is unavailable, stop this path and return only:
   ...
   Do not continue the steps below, fill the ordinary refine result, name a
   perceptual mismatch, or prescribe a treatment as though it had been observed.
2. Find the leading detail mismatch: the repeated relation whose correction
   ...
   icon-bearing or stateful control when they exist. A component library,
   default value, or shared asset is not a mismatch merely because it was not
   custom-made; establish the contradiction in the rendered whole.
3. Form the smallest component-expression map needed for this case. For each
   retained detail family, state the functional role, relation to the direction,
   surfaces affected, and whether the decision is inherited, local, shared, or
   experimental. Explicitly mark details that should remain neutral.
4. Implement the highest-leverage coherent slice through real primitives and
   consumers. Change a shared primitive only when its consumers genuinely need
   the same relation; otherwise keep the treatment local. Preserve semantics,
   interaction contracts, focus, contrast, target size, content density, and
   reduced-motion behavior.
5. Inspect the rendered result beside the prior state at representative content,
   ...
   user's decision. A consistent token table is not evidence of this result.
6. Return:
...
coherence. Supporting details should make the main design legible and continuous;
they do not replace its content relation, hierarchy, or direction.
