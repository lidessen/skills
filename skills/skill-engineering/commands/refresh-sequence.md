...
1. Read `references/sequence.md`. Confirm the repository, ref,
...
4. Use the verified remote material only for the current task and record its
   ...
   Never overwrite the packaged Sequence projection from a task run.
5. If the remote material changes a target-skill selection, surface that as a
   ...
   not overwrite `references/sequence.md` or `references/sequence-interpretations/`.
