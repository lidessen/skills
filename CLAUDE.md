...
- Frontmatter `description` acts as the trigger classifier. Include both the methodology description and concrete trigger phrases/argument hints. **Prefer a single-line YAML string** — a long single line keeps "what it does + when to trigger" in one continuous chunk, which is how trigger classifiers read it. If readability suffers, fall back to YAML *folded* scalar (`description: >`), which still parses to one logical line. Avoid the *literal* form (`description: |`), which preserves line breaks and fragments triggers across short lines. Whatever the form, the value should read as one continuous paragraph, not a bullet list.
